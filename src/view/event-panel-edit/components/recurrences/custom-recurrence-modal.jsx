...
export default function CustomRecurrenceModal({ openModal, setOpenCb, data, callbacks }) {
	...
	const defaultWeekDay = useMemo(() => moment(data.start).format('dddd'), [data]);
	const slicedWeekDay = useMemo(() => toUpper(`${defaultWeekDay.slice(0, 2)}`), [defaultWeekDay]);
	const bymonth = useMemo(
		() => ({ molist: parseInt(moment(data.start).format('MM'), 10) }),
		[data?.start]
	);
	const bymonthday = useMemo(() => ({ modaylist: moment(data.start).format('D') }), [data?.start]);
	...
	}, [callbacks, customEndValue, customRepeatValue, setOpenCb]);
