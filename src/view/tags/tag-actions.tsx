...
export const TagsDropdownItem = ({
	...
	console.log('vvx event:', conversation);
	const [t] = useTranslation();
	const createSnackbar = useContext(SnackbarManagerContext);
	const dispatch = useDispatch();
	const [checked, setChecked] = useState(includes(conversation.resource.tags, tag.id));
	...
	const toggleCheck = useCallback(
		(value) => {
			...
			dispatch(
				itemAction({
					...
					inviteId: conversation.resource.id,
					tagName: tag.name
				})
				// eslint-disable-next-line @typescript-eslint/ban-ts-comment
				// @ts-ignore
		...
		[conversation.resource.id, createSnackbar, dispatch, t, tag.name]
	);
	// eslint-disable-next-line @typescript-eslint/ban-ts-comment
	// @ts-ignore
...
export const MultiSelectTagsDropdownItem = ({
	...
	const toggleCheck = useCallback(
		(value) => {
			setChecked((c) => !c);
			dispatch(
				itemAction({ operation: value ? '!tag' : 'tag', ids, tagName: tag.name })
		...
		[createSnackbar, deselectAll, dispatch, folderId, ids, t, tag.name]
	);
...
export const applyTag = ({
	...
	console.log('vv tags:', context.tags);
	const tagItem = reduce(
		context.tags,
		(acc, v) => {
			const item = {
				...
				icon: 'TagOutline',
				keepOpen: true,
				customComponent: <TagsDropdownItem tag={v} conversation={event} isMessage={false} />
			};
			// eslint-disable-next-line @typescript-eslint/ban-ts-comment
			// @ts-ignore
			acc.push(item);
